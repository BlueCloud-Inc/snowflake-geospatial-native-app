...
def load_app(orders_table):
    with st.spinner("Loading sale locations insights, please wait...."):
        ...
        _ = session.sql(f"CREATE OR REPLACE VIEW orders_v COMMENT = 'Tasty Bytes Order Detail View' AS SELECT DATE(o.order_ts) AS date,o.* ,cpg.* EXCLUDE (location_id, region, phone_number, country) FROM {orders_table} o JOIN frostbyte_tb_safegraph_s cpg ON o.location_id = cpg.location_id;").collect()
